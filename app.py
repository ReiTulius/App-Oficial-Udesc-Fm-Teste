...
if opcao == "🔍 Buscar no Acervo":
    ...
    if df_acervo is not None and not df_acervo.empty:
        ...
        if termo:
            ...
            st.info(f"💡 Banco conectado! Existem {len(df_acervo)} músicas carregadas prontas para pesquisa.")
    else:
        st.warning("⚠️ Não foi possível indexar as planilhas de forma unificada. Verifique se estão públicas para leitura.")
